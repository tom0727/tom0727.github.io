...
3. 对于一个非负整数 $x$，如果在 $S$ 中，取一些元素 $x_1, x_2, ..., x_k$ 使得 $x = XOR(\\{x_1,x_2,...,x_k\\})$，则：
   
   使用 $x$ 替换掉 $x_1, x_2, ..., x_k$ 中的任意一个，线性基均不变。

   > 如何找到这样的 $x_1, x_2, ..., x_k$ 使得 $x = XOR(\\{x_1,x_2,...,x_k\\})$？

   答：在正常的 `insert()` 过程中，如果执行了 `x ^= a[j]`，就说明 $x_j \in \\{x_1,x_2,...,x_k\\}$。
